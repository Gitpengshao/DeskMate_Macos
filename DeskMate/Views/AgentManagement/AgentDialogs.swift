...
// MARK: - Install Distribution Dialog

/// 从 Git 仓库安装 distribution — 对齐 `hermes profile install`。
struct InstallDistributionDialog: View {
    @ObservedObject var viewModel: AgentViewModel
    @Environment(\.dismiss) private var dismiss

    @State private var sourceType: DistributionSourceType = .github
    @State private var source: String = ""
    @State private var localName: String = ""
    @State private var createAlias: Bool = true
    @State private var errorText: String = ""
    ...
    private var header: some View {
        HStack(spacing: 10) {
            Image(systemName: "arrow.down.circle.fill")
                .font(.system(size: 16, weight: .semibold))
                .foregroundColor(AgentPalette.textPrimary)
            Text(AgentText.installDistTitle)
                .font(.system(size: 15, weight: .semibold))
                .foregroundColor(AgentPalette.textPrimary)
            Spacer()
            Button(action: { dismiss() }) {
    ...
    private var sourceTypePicker: some View {
        HStack(spacing: 8) {
            ForEach(DistributionSourceType.allCases) { type in
                Button(action: { sourceType = type }) {
                    HStack(spacing: 5) {
                    ...
                    .foregroundColor(sourceType == type ? AgentPalette.inverseInk : AgentPalette.textPrimary)
                    .padding(.horizontal, 10).padding(.vertical, 5)
                    .background(
                        RoundedRectangle(cornerRadius: 5)
                            .fill(sourceType == type ? AgentPalette.inverse : AgentPalette.bgPanel)
                    )
                    .overlay(
                        RoundedRectangle(cornerRadius: 5)
                            .stroke(AgentPalette.border, lineWidth: 0.5)
    ...
    private var sourceField: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(AgentText.fieldGitSource)
                .font(.system(size: 11, weight: .semibold))
                .tracking(0.4)
                .foregroundColor(AgentPalette.textMuted)
                .textCase(.uppercase)
            TextField(sourceType.placeholder, text: $source)
                .textFieldStyle(.plain)
    ...
    private var localNameField: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(AgentText.fieldLocalName)
                .font(.system(size: 11, weight: .semibold))
                .tracking(0.4)
                .foregroundColor(AgentPalette.textMuted)
                .textCase(.uppercase)
            TextField("默认从 distribution.yaml 读取", text: $localName)
                .textFieldStyle(.plain)
                .font(.system(size: 12, design: .monospaced))
                .foregroundColor(AgentPalette.textPrimary)
                .padding(.horizontal, 10).padding(.vertical, 8)
    ...
    private var optionsRow: some View {
        VStack(alignment: .leading, spacing: 6) {
            Toggle(isOn: $createAlias) {
                HStack(spacing: 5) {
                    Image(systemName: "terminal")
                        .font(.system(size: 10))
                    Text(AgentText.fieldCreateAlias)
                        .font(.system(size: 11.5))
                }
            }
    ...
    private var hintBlock: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("""
• 仓库根目录需包含 distribution.yaml
• 凭据 (.env / auth.json)、记忆与会话不会被覆盖
• 后续可运行 `hermes profile update <name>` 拉取新版
""")
                .font(.system(size: 11))
                .foregroundColor(AgentPalette.textMuted)
                .lineSpacing(2)
    ...
    private var footer: some View {
        HStack {
            Button("查看 distribution 文档") {
                viewModel.openDistributionsDocs()
            }
            .buttonStyle(.plain)
            .font(.system(size: 11, weight: .medium))
            .foregroundColor(AgentPalette.textMuted)
            ...
            Button(AgentText.cancel) { dismiss() }
                ...
                .foregroundColor(AgentPalette.textMuted)
                .padding(.horizontal, 12).padding(.vertical, 6)
            Button(AgentText.install) { submit() }
                .buttonStyle(.plain)
                .font(.system(size: 12, weight: .semibold))
                .foregroundColor(AgentPalette.inverseInk)
                .padding(.horizontal, 14).padding(.vertical, 6)
                .background(
                    RoundedRectangle(cornerRadius: 6)
                        .fill(source.isEmpty ? AgentPalette.textDisabled : AgentPalette.inverse)
                )
    ...
    private func submit() {
        ...
        guard !trimmed.isEmpty else {
            errorText = AgentText.noSource
            return
        }
        errorText = ""
        Task {
            await viewModel.installDistribution(
                source: trimmed,
                name: localName.isEmpty ? nil : localName,
                alias: createAlias
            )
            if viewModel.model.errorMessage.isEmpty {
                dismiss()
