...
public final class DynamicNotch<Expanded, CompactLeading, CompactTrailing>: ObservableObject, DynamicNotchControllable where Expanded: View, CompactLeading: View, CompactTrailing: View {
    /// Explicit ObservableObject publisher for generic class compatibility
    ...
    /// 当为 true 时，鼠标悬浮不会触发 compact → expanded 展开。
    /// 用于业务层在控制台打开期间禁用悬浮展开，控制台关闭后恢复。
    public var disableHoverExpand: Bool = false
    ...
    func updateHoverState(_ hovering: Bool) {
        ...
        // 鼠标悬浮时立即从 compact 展开到 expanded（带动画过渡）
        // 当 disableHoverExpand 为 true 时（如控制台已打开），禁止悬浮展开
        if hovering, state == .compact, !disableHoverExpand {
            Task { @MainActor in
                await self.expand()
            }
        }

        // 鼠标离开时延迟收起以避免死循环（带动画过渡）
        if !hovering, state == .expanded {
            Task { @MainActor in
                try? await Task.sleep(for: .seconds(0.15))
                guard !self.isHovering, self.state == .expanded else { return }
                await self.compact()
            }
        }
...
extension DynamicNotch {
    ...
    func _expand(on screen: NSScreen = NSScreen.screens[0], skipHide: Bool) async {
        ...
        if needsNewWindow {
            ...
            // 创建全新的 NSHostingView 以确保 SwiftUI 内容在状态变化后正确渲染
            if let panel = windowController?.window {
                let style = effectiveStyle(for: screen)
                let newView = NSHostingView(rootView: NotchContentView(dynamicNotch: self, style: style))
                newView.frame = panel.contentView?.frame ?? .zero
            ...
            // 直接从 compact 平滑过渡到 expanded（带动画）
            Task { @MainActor in
                withAnimation(self.effectiveConversionAnimation) {
                    self.state = .expanded
                }

                // 创建全新的 NSHostingView 以确保 SwiftUI 内容在状态变化后正确渲染
                if let panel = self.windowController?.window {
                    let style = self.effectiveStyle(for: screen)
                    let newView = NSHostingView(rootView: NotchContentView(dynamicNotch: self, style: style))
                    newView.frame = panel.contentView?.frame ?? .zero
                    panel.contentView = newView
                }
    ...
    func _compact(on screen: NSScreen = NSScreen.screens[0], skipHide: Bool) async {
        ...
        if needsNewWindow {
            ...
            // 直接从 expanded 平滑过渡到 compact（带动画）
            Task { @MainActor in
                withAnimation(self.effectiveConversionAnimation) {
                    self.state = .compact
                }
